...
```{r setup}
#knitr::opts_knit$set(root.dir = "~/KCNISS_2023/astro_pseudobulk")
```
...
pseudobulk_seu_10x <- AggregateExpression(object = Seu_10x, group.by = c('external_donor_name_label', 
                                                                                'subclass_label'),
                                                 slot = 'counts')
...
head(pseudobulk_seu_10x_counts)
```
...
ggplot(sea_ad_meta,
  ...
  stat_cor(show.legend = F) #note, if you don't want a specific item of your ggplot to be 
                            #included in the legend, use show.legend=F
```
...
# not that when a joining variable is not specified to left_join, it will join by the first column name match
```
