...
```{r setup}
knitr::opts_knit$set(root.dir = "~/KCNISS_2023/SST_pseudobulk/data")
```
...
We will only be working with one cell type: Astrocytes. 


### Load pseudobulked gene expression data

```{r}
pseudobulk_astro_counts = read_csv('/external/rprshnas01/netdata_kcni/stlab/SEA-AD/astro_pseudobulk.csv')
# pseudobulk_sst_counts = read_csv('SST_pseudobulk.csv')
gene_info = read_csv('/external/rprshnas01/netdata_kcni/stlab/SEA-AD/astro_pseudobulk_gene_info.csv')
sea_ad_meta = read_csv('/external/rprshnas01/netdata_kcni/stlab/SEA-AD/astro_pseudobulk_meta_updated.csv')
...
It looks like there are more Astrocytes in individuals with dementia than controls
...
pseudobulk_astro_cpm_trans = left_join(sea_ad_meta, pseudobulk_astro_cpm_trans %>% 
                                       rownames_to_column(var = 'donor_id')) 
```

### Plot gene expression of a gene (GFAP below) vs. all cell types
```{r}
p1 = pseudobulk_astro_cpm_trans %>% 
  filter(`Neurotypical reference` == F) %>%
...
library_size = pseudobulk_astro_counts[, use_subjects] %>% colSums %>% unlist()
...
```{r}
dge0 = DGEList(pseudobulk_astro_counts[, use_subjects], genes = gene_info)
...
```{r}
col8a1_plot = pseudobulk_astro_cpm_trans %>% 
  filter(donor_id %in% use_subjects_donor_list) %>%
  ggplot(aes(x = disease, y = log2(COL8A1+1), fill = disease) ) + 
  ylab('COL8A1 mRNA expr (log2 CPM+1)') + 
  geom_boxplot() + 
  stat_compare_means() + 
  theme_cowplot() 

lrrc7_plot = pseudobulk_astro_cpm_trans %>% 
  filter(donor_id %in% use_subjects_donor_list) %>%
  ggplot(aes(x = disease, y = log2(`LRRC7-AS1`+1), fill = disease)) + 
...
col8a1_plot = pseudobulk_astro_cpm_trans %>% 
  filter(donor_id %in% use_subjects_donor_list) %>%
  ggplot(aes(x = `Continuous Pseudo-progression Score`, y = log2(COL8A1+1), color = disease, group = 1) ) + 
  ylab('COL8A1 mRNA expr (log2 CPM+1)') + 
  xlab('AD Pathology score') + 
  geom_point() + 
  geom_smooth(method = 'lm', se = F) + 
...
slc28a2_plot = pseudobulk_astro_cpm_trans %>% 
  filter(donor_id %in% use_subjects_donor_list) %>%
  ggplot(aes(x = `Continuous Pseudo-progression Score`, y = log2(SLC28A2+1), color = disease, group = 1)) + 
